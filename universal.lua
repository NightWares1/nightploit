		...
		if tostring(string.lower(NameCallMethod)) == "kick" then
			if n == nil then
				game.StarterGui:SetCore(
					"SendNotification",
					{ Title = "Night Hub", Text = "Kick prevented.", Duration = 2 }
				)
				print("[Night Hub] Kick prevented.")
			else
				game.StarterGui:SetCore(
					"SendNotification",
					{ Title = "Night Hub", Text = "Kick " .. '"' .. n .. '"' .. " prevented.", Duration = 2 }
				)
				print("[Night Hub] Kick " .. '"' .. n .. '"' .. " prevented.")
			end
			return nil
		end
...
function NightUI.new()
	...
	ExitButton.MouseButton1Click:Connect(function()
		...
		wait(1)
		GUI:Destroy()
		warn('[Night Hub] Terminating Threads (ignore errors like "attempt to call a nil value")')
		Threads:Cleanup()
		script:Destroy()
	end)
...
function NightUI:addPage(title: string)
...
function NightUI:Notify(text: string, mode: number)
	if mode == nil or mode == 0 then
		print("[Night Hub] " .. text)
	elseif mode == 1 then
		warn("[Night Hub] " .. text)
	end
...
-- Init
NightHub = NightUI.new()

-- Local Player Page
local LocalPlayerPage = NightHub:addPage("Local Player")
...
-- Universal Page
local UniversalPage = NightHub:addPage("Universal")
...
-- merge Night Dex with Secure dex
-- Night Dex
UniversalPage:addButton("Night Dex", function()
	local Dex = rawget(game:GetObjects("rbxassetid://8126316565"), 0X1)
	if type(syn) == "table" and type(syn.protect_gui) == "function" then
		xpcall(syn.protect_gui, warn, Dex)
	end
	Dex.Name, Dex.Parent = "Night Dex", game:GetService("CoreGui")
...
LocalPlayer.Idled:Connect(function()
	if AntiAFKButton:IsEnabeld() then
		...
		NightHub:Notify("Reflected idle Kick")
	end
end)
...
if setclipboard ~= nil then
	local CoppyPage = NightHub:addPage("Coppy")
...
if Drawing ~= nil then
	...
	local Visuals = NightHub:addPage("Visuals")
...
pcall(function()
	local games = HttpService:JSONDecode(
		game:HttpGet("https://raw.githubusercontent.com/Commandcracker/Night-Hub/main/games.json")
	)
	local GamesPage = NightHub:addPage("Games")
...
local successed, errData = pcall(function()
	loadstring(
		game:HttpGet("https://raw.githubusercontent.com/Commandcracker/Night-Hub/main/games/" .. game.GameId .. ".lua")
	)()
end)
...
if successed then
	...
	NightHub:Notify("Game Supported")
elseif errData ~= nil then
	if errData == "HTTP 404 (Not Found)" then
		NightHub:Notify("Game Not Supported")
	else
		NightHub:Notify("Failed To load game scripts", 1)
		warn(errData)
	end
end
